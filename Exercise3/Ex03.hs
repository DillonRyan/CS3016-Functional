...
insBST x y (Branch left k v right)
  | x < k = Branch (insBST x y left) k v right
  | x > k = Branch left k v (insBST x y right)
  | otherwise = Branch left x y right
...
bst2assoc Leaf = []
bst2assoc (Branch left x y right) = concat[bst2assoc left, [(x,y)], bst2assoc right]
